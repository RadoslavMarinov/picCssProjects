...
if (IPGetHeader(&dest_ip, &node, &prot, &len)) {
   printf("\r\n DEST: %U.%U.%U.%U SRC: %U.%U.%U.%U LEN: %LU PROT: %X ",dest_ip.v[0], dest_ip.v[1], dest_ip.v[2], dest_ip.v[3],node.IPAddr.v[0], node.IPAddr.v[1], node.IPAddr.v[2], node.IPAddr.v[3],len, prot);
...
int8 type;
   ...
   printf("MY_IP: %u.%u.%u.%u\n\r",MY_IP_BYTE1,MY_IP_BYTE2,MY_IP_BYTE3,MY_IP_BYTE4);
   MACAddrInit();
   MACInit();
